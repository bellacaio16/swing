...
TEST_MODE = True
TEST_DATE = datetime.date(2025, 3, 9)  # fixed date for consistent caching
...
def fetch_candles(smart, symbol, token, start_date, end_date, interval):
    ...
    for attempt in range(3):
        try:
            ...
            logger.warning(f"[{symbol}] DataException on {interval}")
            break
...
def analyze_stock(smart, symbol, token):
    ...
    for i in range(1, ANALYSIS_DAYS+1):
        ...
        for ts, row in day_df.iterrows():
            ...
            bp = lvl
            sl = bp * (1 - SL_BUFFER)
            higher = [l for l in levels if l['level'] > bp]
            higher_lvls = [l['level'] for l in higher]
            t1 = higher_lvls[0] if len(higher_lvls)>=1 else bp*(1+TARGET1_PCT)
            t2 = higher_lvls[1] if len(higher_lvls)>=2 else bp*(1+TARGET2_PCT)
            t3 = higher_lvls[2] if len(higher_lvls)>=3 else bp*(1+TARGET3_PCT)

            sl, t1, t2, t3 = round_price(sl), round_price(t1), round_price(t2), round_price(t3)
            candidates.append({
                'date': day, 'time': ts.time(),
                'symbol': symbol, 'token': token,
                'action': 'BUY', 'breakout_level': round_price(bp),
